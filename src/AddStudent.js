...
const StudentValidatOnSchema = yup.object({
  ...
  Class: yup.string().required("Tell something about this person").min(4, "min 4 character"),
...
export function AddStudent() {
  ...
  return (<>
    <form onSubmit={handleSubmit} className='add-teach'>
      <TextField label="Student" variant="outlined" name='name' type='text' value={values.name}
        onChange={handleChange} onBlur={handleBlur} error={touched.name && errors.name}
        helperText={touched.name && errors.name ? errors.name : null} teacher="name" />
      <TextField
        name='Class' onChange={handleChange} value={values.Class} error={touched.Class && errors.Class}
        helperText={touched.Class && errors.Class ? errors.Class : null}
        onBlur={handleBlur} label="Class" variant="outlined" type='text' />
      <TextField
